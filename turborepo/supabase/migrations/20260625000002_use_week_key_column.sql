-- ============================================================
-- Migration: use stored week_key column in views
--
-- Replaces the inline split_part(camp_id, '_', 4) expression
-- in both views with the stored week_key column on camp_sessions.
-- The stored column is indexable; the inline expression is not.
--
-- camp_sessions_scored: adds week_key as a direct pass-through.
-- best_camp_sessions:   partitions by cs.week_key and selects it
...
comment on view camp_sessions_scored is
  ...
  'completeness_pct (0–100). Joins research_runs for run_date. week_key '
  'is now a direct column pass-through (no longer derived via split_part). '
...
with ranked as (
  select
    *,
    -- Natural key group: org + name + type + week_key (stored column, indexable).
    row_number() over (
      partition by
        organization_id,
        camp_name,
...
comment on view best_camp_sessions is
  ...
  'week_key). week_key is now read from the stored column — no split_part. '
  'Best row = highest completeness_score; tiebreak = latest run_date.';
